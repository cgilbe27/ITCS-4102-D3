(function() {
  ...
  function d3_interpolateCubehelix(f) {
    return function(a, b) {
      a = d3.hsl(a);
      b = d3.hsl(b);
      ...
      return function(t) {
        var h = ah + bh * t,
            l = Math.pow(al + bl * t, f),
            a = (as + bs * t) * l * (1 - l);
        return "#"
            + hex(l + a * (-0.14861 * Math.cos(h) + 5.08277 * Math.sin(h)))
            + hex(l + a * (-5.29227 * Math.cos(h) - 0.00649 * Math.sin(h)))
            + hex(l + a * (+5.97294 * Math.cos(h)));
      };
    };
  }
